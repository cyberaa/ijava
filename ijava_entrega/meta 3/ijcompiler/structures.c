...
AST_TREE* add_VarDecl(int type, char* id, ID_List* multi_IDs, AST_TREE* next){
    ...
    newVar->tipo_no = NO_VARDECL;
    newVar->type = (Tipo)type;
...
AST_TREE* add_Method(int type, char* id, AST_TREE* params, AST_TREE* varDecl, AST_TREE* statements){
    ...
    add_Method->tipo_no = NO_METHODDECL;
...
AST_TREE* add_Compound_Stmnt(AST_TREE* Expr){
    ...
    newCompound->tipo_no = NO_COMPOUNDSTAT;
    newCompound->n1 = Expr;
...
AST_TREE* add_Store(char* id, AST_TREE* arrayIndex, AST_TREE* Expr){
    ...
    if(arrayIndex!=NULL){
        thisNode->tipo_no = NO_STOREARRAY;
        if(arrayIndex == NULL)
            thisNode->n2 = create_Root_node();
    }else
        thisNode->tipo_no = NO_STORE;
...
AST_TREE* add_Terminal_Node(int tipo_no, char* token){

    AST_TREE* newTerminal = (AST_TREE*) calloc(sizeof(AST_TREE),1);
    if(newTerminal == NULL){
        assert(newTerminal != NULL);
    }
    newTerminal->tipo_no = tipo_no;
    if(tipo_no == NO_INTLIT)
        newTerminal->type = TYPE_INT;
    else if(tipo_no == NO_BOOLLIT)
...
AST_TREE* add_ParseInt(char* id, AST_TREE* indx_Expr){
    ...
    newParseInt->tipo_no = NO_PARSEARGS;
...
AST_TREE* add_DotLength(AST_TREE* Expr){
    ...
    newDotLength->tipo_no = NO_LENGTH;
...
AST_TREE* add_LoadArray(AST_TREE* Expr, AST_TREE* indx_Expr){
    ...
    newLoadArray->tipo_no = NO_LOADARRAY;
...
AST_TREE* add_two_Expr(AST_TREE* exp1,char* op,AST_TREE* exp2){
    ...
    newExpression-> tipo_no = getOPType(op);


    if(newExpression->tipo_no == -1 )
        exit(-2);
...
AST_TREE* add_Expr(char* op,AST_TREE* exp){
    ...
    if(newExpression->tipo_no == NO_PLUS)
        newExpression->tipo_no = NO_UNARYPLUS;
    if(newExpression->tipo_no == NO_MINUS)
        newExpression->tipo_no = NO_UNARYMINUS;
...
Sym_Table* create_SymTable(AST_TREE* ast){
    ...
    if(ast!=NULL && ast->tipo_no == NO_PROGRAMA){
        if(mainTable == NULL){
            //create the table
    ...
    while(ast!=NULL){
        if( ast->tipo_no == NO_VARDECL){
            symbol = add_NewDecl_Table_to_Old(0, symbol,ast, mainTable);
            ast = ast->next;
        }
        else if( ast->tipo_no == NO_METHODDECL){
            //Create Sym Table
...
void check_Sym_Exists(char* id, Sym_Table* table){
    ...
    while(local!=NULL){
        if((local->tipo_no == TABLE_DECL || local->tipo_no == TABLE_METHOD) && strcmp(id,local->id->id)==0){
...
int check_Sym_ID_Exists(char* id,Symbol_Tipo type, Sym_Table* table, Sym_Table* main){
    Sym_Node* local = table->table->next;
    while(local!=NULL){
        if((local->tipo_no == type) && strcmp(id,local->id->id)==0){
            return local->type;
        }
        local = local->next;
    }
...
Sym_Table* get_Method_SymTable(Sym_Table* main, char* methodID){
    while(main!=NULL){
        if(main->table->tipo_no == TABLE_METHOD && strcmp(methodID,main->table->id->id)==0)
            return main;
        else
            main = main->next;
   }
...
void check_Types(AST_TREE* ast, Sym_Table *main){
    ...
    if(ast->tipo_no == NO_GREATER || ast->tipo_no == NO_LESS || ast->tipo_no == NO_GREATEREQUAL || ast->tipo_no == NO_LESSEQUAL ){
        if(ast->n1->type != TYPE_INT || ast->n2->type != TYPE_INT){
            OP_Error_two_Types(ast->tipo_no,ast->n1->type,ast->n2->type);
        }
        ast->type = TYPE_BOOL;
    }
    else if(ast->tipo_no == NO_PLUS || ast->tipo_no == NO_MINUS || ast->tipo_no == NO_DIV || ast->tipo_no == NO_MUL || ast->tipo_no == NO_MOD){
        if(ast->n1->type != TYPE_INT || ast->n2->type != TYPE_INT){
            OP_Error_two_Types(ast->tipo_no,ast->n1->type,ast->n2->type);
        }
        ast->type = TYPE_INT;
    }
    else if(ast->tipo_no == NO_EQUAL || ast->tipo_no == NO_DIFFERENT){
        if(ast->n1->type != ast->n2->type){
            OP_Error_two_Types(ast->tipo_no,ast->n1->type,ast->n2->type);
        }
        ast->type = TYPE_BOOL;
    }
    else if(ast->tipo_no == NO_AND || ast->tipo_no == NO_OR){
        if(ast->n1->type != TYPE_BOOL || ast->n2->type != TYPE_BOOL){
            OP_Error_two_Types(ast->tipo_no,ast->n1->type,ast->n2->type);
        }
        ast->type = TYPE_BOOL;
    }
    else if(ast->tipo_no == NO_LOADARRAY ){
    ...
    else if( ast->tipo_no == NO_NEWINT || ast->tipo_no == NO_NEWBOOL ){
        assert(ast->n1!=NULL);
        if(ast->n1->type != TYPE_INT){
            OP_Error_Type(ast->tipo_no,ast->n1->type);
        }
        if(ast->tipo_no == NO_NEWINT)
            ast->type = TYPE_INT_ARRAY;
        else
            ast->type = TYPE_BOOL_ARRAY;
    }
    else if( ast->tipo_no == NO_LENGTH){
        if((ast->n1->type != TYPE_BOOL_ARRAY && ast->n1->type != TYPE_INT_ARRAY && ast->n1->type != TYPE_STRING_ARRAY)){
            OP_Error_Type(ast->tipo_no,ast->n1->type);
        }
        ast->type = TYPE_INT;
    }
    else if( ast->tipo_no == NO_UNARYPLUS || ast->tipo_no == NO_UNARYMINUS){
        if((ast->n1->type != TYPE_INT)){
            OP_Error_Type(ast->tipo_no,ast->n1->type);
        }
        ast->type = TYPE_INT;
    }
    else if(ast->tipo_no == NO_NOT){
        if((ast->n1->type != TYPE_BOOL)){
            OP_Error_Type(ast->tipo_no,ast->n1->type);
        }
        ast->type = TYPE_BOOL;
    }
    else if( ast->tipo_no == NO_PARSEARGS){
        if((ast->n1->type != TYPE_STRING_ARRAY || ast->n2->type != TYPE_INT)){
            OP_Error_two_Types(ast->tipo_no,ast->n1->type,ast->n2->type);
        }
        ast->type = TYPE_INT;
    }
    else if (ast->tipo_no == NO_CALL){
        int i = 0;
        Tipo astType, tableType;
    ...
    //Statement 
    else if( ast->tipo_no == NO_IFELSE || ast->tipo_no == NO_WHILE){
        if((ast->n1->type != TYPE_BOOL)){
            statmnt_Error(ast->tipo_no,ast->n1->type,TYPE_BOOL);
        }
    }
    else if( ast->tipo_no == NO_PRINT){
        if((ast->n1->type != TYPE_BOOL && ast->n1->type != TYPE_INT)){
            statemnt_Error_3vars(ast->tipo_no,ast->n1->type,TYPE_BOOL,TYPE_INT);
        }
        ast->type = TYPE_INT;
    }else if(ast->tipo_no == NO_COMPOUNDSTAT){
        ast->type = ast->n1->type;
    }
    else if(ast->tipo_no == NO_STORE || ast->tipo_no == NO_STOREARRAY){
    ...
    else if( ast->tipo_no == NO_RETURN){
        if(ast->n1!=NULL){
            if((ast->n1->type != get_Function_Types())){
                statmnt_Error(ast->tipo_no,ast->n1->type,get_Function_Types());
            }
        }else if(get_Function_Types()!=TYPE_VOID){
            statmnt_Error(ast->tipo_no,TYPE_VOID,get_Function_Types());
        }
    }
}
